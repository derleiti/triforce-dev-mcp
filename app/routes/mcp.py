...
<div class="row"><button id="chooseBtn">Choose local folder</button><label id="fallbackLabel" for="fallbackPicker" class="choice hidden" style="cursor:pointer;font-weight:650">Choose local folder</label><span id="folderName" class="muted">No folder selected</span></div>
<input id="fallbackPicker" type="file" webkitdirectory directory multiple class="file-hidden" aria-label="Choose local workspace folder">
<div id="dropZone" class="choice" style="margin-top:10px;border-style:dashed;text-align:center">Desktop fallback: drop a project folder here</div>
...
const isFirefox=/Firefox\//.test(navigator.userAgent);const modern=!isFirefox&&typeof window.showDirectoryPicker==='function';
if(!modern){$('writeMode').disabled=true;$('chooseBtn').classList.add('hidden');$('fallbackLabel').classList.remove('hidden');$('browserNote').textContent=(isFirefox?'Firefox mode: ':'Browser fallback: ')+'choose the folder with the native directory picker. Reading/search is supported; direct Write access requires a browser with the File System Access API.'}else{$('fallbackLabel').classList.add('hidden');$('browserNote').textContent='This browser supports direct folder access. Write mode asks for explicit read/write permission.'}
...
async function chooseFolder(){disconnect();$('pairPanel').classList.add('hidden');if(!modern)return;try{rootHandle=await window.showDirectoryPicker({mode:'read'});fallbackFiles.clear();fallbackRoot='';$('folderName').textContent=rootHandle.name||'Selected folder';$('connectBtn').disabled=false;status('Folder selected. Choose access and connect.','ok');$('analysis').textContent='Folder ready. Project analysis will run after pairing or when requested.';}catch(e){if(e.name!=='AbortError')status('Folder selection failed: '+e.message,'warn')}}
function installFallbackFiles(files){fallbackFiles.clear();rootHandle=null;const list=[...(files||[])];if(!list.length)return false;const firstRel=String(list[0].webkitRelativePath||list[0]._relativePath||'');fallbackRoot=firstRel.includes('/')?firstRel.split('/')[0]:'Selected folder';for(const f of list){let p=String(f.webkitRelativePath||f._relativePath||f.name||'');if(fallbackRoot!=='Selected folder'&&p.startsWith(fallbackRoot+'/'))p=p.slice(fallbackRoot.length+1);p=cleanPath(p||f.name);if(p)fallbackFiles.set(p,f)}if(!fallbackFiles.size)return false;$('folderName').textContent=fallbackRoot+' · '+fallbackFiles.size+' files';document.querySelector('input[name=mode][value=read_only]').checked=true;$('connectBtn').disabled=false;status('Folder selected. Ready to connect.','ok');$('analysis').textContent=fallbackFiles.size+' local files available. Full analysis will run only when the AI requests workspace_info.';return true;}
async function acceptFallbackSelection(input){status('Reading selected folder…');for(const delay of [0,50,200,600]){if(delay)await new Promise(r=>setTimeout(r,delay));const files=[...(input.files||[])];if(installFallbackFiles(files))return}const entries=[...(input.webkitEntries||[])];if(entries.length)$('analysis').textContent='Browser returned '+entries.length+' directory entries but no FileList.';status('The browser opened the folder picker but did not expose the selected folder files to this page. On desktop, drop the folder into the box below. On mobile, this browser/system picker may not support browser workspace folders.','warn');}
$('fallbackPicker').addEventListener('change',e=>acceptFallbackSelection(e.currentTarget));
...
const dz=$('dropZone');dz.addEventListener('dragover',e=>{e.preventDefault();dz.style.borderColor='#58a6ff'});dz.addEventListener('dragleave',()=>dz.style.borderColor='');dz.addEventListener('drop',async e=>{e.preventDefault();dz.style.borderColor='';const items=[...(e.dataTransfer?.items||[])];const roots=items.map(i=>i.webkitGetAsEntry?.()).filter(Boolean);if(roots.length){const files=[];for(const entry of roots)await filesFromEntry(entry,'',files);if(installFallbackFiles(files))return}if(installFallbackFiles([...(e.dataTransfer?.files||[])]))return;status('No usable files were provided by the dropped folder.','warn')});
