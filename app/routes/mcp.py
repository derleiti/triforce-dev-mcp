...
function startHeartbeat(){stopHeartbeat();lastPongAt=Date.now();const ping=()=>{if(ws&&ws.readyState===WebSocket.OPEN){try{ws.send(JSON.stringify({jsonrpc:'2.0',method:'ping',params:{ts:Date.now()}}))}catch{}}};ping();heartbeatTimer=setInterval(ping,10000);watchdogTimer=setInterval(()=>{if(document.hidden||!ws||ws.readyState!==WebSocket.OPEN)return;if(lastPongAt&&Date.now()-lastPongAt>30000){status('Workspace heartbeat lost. Reconnecting…','warn');try{ws.close(4000,'heartbeat timeout')}catch{}}},5000)}
