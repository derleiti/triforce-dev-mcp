...
const EXECUTOR_VERSION='2.86.6-browser';
...
async function clearWorkspace(args){if(workspaceMode!=='write'||!rootHandle)throw new Error('workspace is read-only');if(String(args.confirm||'')!=='DELETE_ALL')throw new Error('workspace_clear requires confirm=DELETE_ALL');await ensureWritePermission();const names=[];for await(const [name] of rootHandle.entries())names.push(name);const removed=[],failed=[];for(const name of names){try{await removeEntryCompat(rootHandle,name,true);removed.push(name)}catch(e){failed.push({entry:name,error:String(e?.name||'Error')+': '+String(e?.message||e)})}}if(failed.length&&!removed.length)throw new Error('workspace_clear removed nothing: '+failed.map(f=>f.entry+' ('+f.error+')').join('; '));return {ok:failed.length===0,removed_entries:removed.length,removed,failed,root_preserved:true};}
...
async def download_android_workspace_helper() -> Response:
    apk_path = os.getenv("AILINUX_ANDROID_WORKSPACE_APK", "/home/zombie/triforce/releases/android/AILinux-Workspace-latest.apk")
    if not os.path.isfile(apk_path):
        return JSONResponse(
            {"error": "Android helper APK is not built yet", "build": "android_workspace", "version": "2.86.6"},
            status_code=404,
            headers={"Cache-Control": "no-store"},
        )
    return FileResponse(
...
async def workspace_pwa_manifest() -> JSONResponse:
    return JSONResponse({
        "name": "AILinux Workspace",
        "short_name": "AILinux Workspace",
        "id": "/v1/mcp",
        "start_url": "/v1/mcp",
        "scope": "/v1/mcp",
        "display": "standalone",
