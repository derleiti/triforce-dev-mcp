...
function loadWebShareProfile(){try{return {...{clipboardRead:false,clipboardWrite:false,screenObserve:false,resourceAdvertise:false},...JSON.parse(localStorage.getItem(WEB_PROFILE_KEY)||'{}')}}catch{return {clipboardRead:false,clipboardWrite:false,screenObserve:false,resourceAdvertise:false}}}
...
function webSupport(){return {display:Boolean(navigator.mediaDevices&&typeof navigator.mediaDevices.getDisplayMedia==='function'),clipboardRead:Boolean(window.isSecureContext&&navigator.clipboard&&typeof navigator.clipboard.readText==='function'),clipboardWrite:Boolean(window.isSecureContext&&navigator.clipboard&&typeof navigator.clipboard.writeText==='function'),deviceInfo:true,webgpu:Boolean(navigator.gpu),python:Boolean(window.Worker&&window.WebAssembly)}}
function webShareNativeProfile(){const s=webSupport();return {clipboard:{read:s.clipboardRead&&webShareProfile.clipboardRead===true,write:s.clipboardWrite&&webShareProfile.clipboardWrite===true},display:{observe:s.display&&webShareProfile.screenObserve===true,control:false},device:{observe:s.deviceInfo&&webShareProfile.resourceAdvertise===true,control:false},resources:{advertise:s.deviceInfo&&webShareProfile.resourceAdvertise===true},compute:{advertise:false,available:false,runtime:'browser-worker'},mcp:{advertise:false}}}
...
function syncNativeShareUi(){const p=nativeHelper?nativeShareProfile:webShareNativeProfile(),c=p.clipboard||{},d=p.display||{},v=p.device||{},r=p.resources||{},x=p.compute||{},m=p.mcp||{},support=webSupport();$('shareClipboardRead').checked=nativeHelper?c.read===true:webShareProfile.clipboardRead===true;$('shareClipboardWrite').checked=nativeHelper?c.write===true:webShareProfile.clipboardWrite===true;$('shareDisplay').checked=nativeHelper?d.observe===true:webShareProfile.screenObserve===true;$('shareSystemObserve').checked=v.observe===true;$('shareSystemControl').checked=v.control===true;$('shareComputerControl').checked=d.control===true;$('shareResources').checked=nativeHelper?r.advertise===true:webShareProfile.resourceAdvertise===true;$('shareCompute').checked=x.advertise===true;$('shareMcp').checked=m.advertise===true;if(!nativeHelper){$('shareClipboardRead').disabled=!support.clipboardRead;$('shareClipboardWrite').disabled=!support.clipboardWrite;$('shareDisplay').disabled=!support.display;$('shareResources').disabled=!support.deviceInfo;for(const id of ['shareSystemObserve','shareSystemControl','shareComputerControl','shareCompute','shareMcp'])$(id).disabled=true;$('saveNativeShares').disabled=false;$('refreshResources').disabled=false}else{$('shareCompute').disabled=x.available===false}}
async function refreshNativeHelperStatus(){const nativeTools=await nativeHelperTools(),tools=[...new Set([...nativeTools,...browserHelperTools()])];syncNativeShareUi();const browserTools=browserHelperTools();if(nativeHelper){$('helperNote').textContent='Native broker active'+(browserTools.length?' · browser fallback also active':'')+' · shared: '+(tools.length?tools.join(', '):'none')+'. All grants are opt-in.'}else{$('helperNote').textContent='Browser capability broker active · available APIs are detected at runtime. Shared now: '+(browserTools.length?browserTools.join(', '):'none')+'. Install the native Helper only for OS capabilities the browser does not expose.'}return tools}
...
async function revokeAllShare(){webShareProfile={clipboardRead:false,clipboardWrite:false,screenObserve:false,resourceAdvertise:false};saveWebShareProfile();stopWebDisplayStream();if(nativeHelper&&typeof nativeHelper.setShareProfile==='function'){nativeShareProfile=await nativeHelper.setShareProfile({clipboardRead:false,clipboardWrite:false,screenObserve:false,systemObserve:false,systemControl:false,computerControl:false,resourceAdvertise:false,computeAdvertise:false,mcpAdvertise:false})||{};}capabilities=[];renderShareSummary();await disconnect(true);syncNativeShareUi();status('All workspace and device capability grants revoked.','warn')}
...
{const w=webSupport();$('browserCaps').textContent='Origin: '+location.origin+' · secure='+(window.isSecureContext?'yes':'no')+' · directory-picker='+(directPicker?'yes':'no')+' · persistent-handle='+(persistentHandleStore?'yes':'no')+' · display-capture='+(w.display?'yes':'no')+' · clipboard-read='+(w.clipboardRead?'yes':'no')+' · clipboard-write='+(w.clipboardWrite?'yes':'no')+' · webgpu='+(w.webgpu?'yes':'no')+' · python-wasm='+(w.python?'yes':'no')+' · wake-lock='+(('wakeLock' in navigator)?'yes':'no')+' · os='+detectedHelperOs();}$('wakeLockToggle').checked=wakeLockWanted;
