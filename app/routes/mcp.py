...
async function execute(tool,args){if(!capabilities.includes(tool))throw new Error('tool not available in this workspace: '+tool);if(tool==='workspace_info')return result(await workspaceInfo());if(tool==='computer_observe'||tool==='computer_screenshot'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.screenshot())}if(tool==='shell'){if(!nativeHelper||typeof nativeHelper.runCompute!=='function')throw new Error('native Docker compute bridge unavailable');return result(await nativeHelper.runCompute(args))}if(tool==='clipboard_read'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardRead())}if(tool==='clipboard_write'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardWrite(String(args.text||'')))}if(tool==='file_read'||tool==='code_read')return result({path:cleanPath(args.path),...lineSlice(await readText(args.path),args)});if(tool==='file_tree'||tool==='code_tree')return result(await tree(args));if(tool==='code_search')return result(await search(args,false));if(tool==='code_grep')return result(await search(args,true));if(tool==='file_edit')return result(await editFile(args));if(tool==='directory_create')return result(await createDir(args));if(tool==='workspace_clear')return result(await clearWorkspace(args));if(tool==='file_ops')return result(await fileOps(args));if(tool==='code_edit')return result(await codeEdit(args));throw new Error('unsupported browser tool');}
