...
<input id="folderListPicker" type="file" webkitdirectory directory multiple style="position:absolute;width:1px;height:1px;opacity:0;pointer-events:none" tabindex="-1" aria-hidden="true">
<div id="dropZone" class="choice" style="margin-top:10px;border-style:dashed;text-align:center">Desktop fallback: drag a local project folder here (nothing is uploaded)</div>
<div class="grid" style="margin-top:14px"><label class="choice"><input type="radio" name="mode" value="read_only" checked> Read only</label><label class="choice"><input id="writeMode" type="radio" name="mode" value="write"> Write <span class="muted">(direct handle)</span></label></div>
...
let rootHandle=null, fallbackFiles=new Map(), fallbackFileList=null, fallbackRoot='', ws=null, workspaceMode='read_only', capabilities=[];
...
const android=/Android/i.test(navigator.userAgent);const firefox=/Firefox\//i.test(navigator.userAgent);
if(!modern){$('writeMode').disabled=true;$('browserNote').textContent=(firefox?'Firefox has no direct writable directory-handle API. ':'')+'Read-only folder selection uses a local FileList fallback. The selected files stay on this device; only explicitly requested files are read. For browser Write access use a Chromium browser with direct directory handles.'}else{$('browserNote').textContent='Direct local folder sharing is supported. Choose Read only or Write before selecting the folder; Write requests read/write permission from the browser. No project analysis runs during selection.'}
$('browserCaps').textContent='Browser filesystem support: direct-handle='+(modern?'yes':'no')+' · local-filelist='+('webkitdirectory' in $('folderListPicker')?'yes':'no')+' · input-showPicker='+(typeof $('folderListPicker').showPicker==='function'?'yes':'no')+' · android='+(android?'yes':'no')+' · firefox='+(firefox?'yes':'no');
...
async function ensureFallbackIndex(){if(fallbackFiles.size||!fallbackFileList||!fallbackFileList.length)return;for(let i=0;i<fallbackFileList.length;i++){const f=fallbackFileList.item?fallbackFileList.item(i):fallbackFileList[i];if(!f)continue;let p=String(f.webkitRelativePath||f._relativePath||f.name||'');if(fallbackRoot&&fallbackRoot!=='Selected folder'&&p.startsWith(fallbackRoot+'/'))p=p.slice(fallbackRoot.length+1);p=cleanPath(p||f.name);if(p)fallbackFiles.set(p,f);if(i&&i%1000===0)await new Promise(r=>setTimeout(r,0));}}
async function fallbackEntries(){await ensureFallbackIndex();return [...fallbackFiles.entries()].filter(([p])=>!ignored(p)).map(([path,file])=>({path,file}));}
async function allFiles(){return rootHandle?await modernFiles():await fallbackEntries();}
async function fileObj(path){const p=cleanPath(path);if(rootHandle){const h=await getHandle(p,true,false);return await h.getFile();}await ensureFallbackIndex();const f=fallbackFiles.get(p);if(!f)throw new Error('file not found: '+p);return f;}
...
async function chooseFolder(){disconnect();$('pairPanel').classList.add('hidden');workspaceMode=document.querySelector('input[name=mode]:checked').value;if(!modern){$('folderListPicker').value='';try{if(typeof $('folderListPicker').showPicker==='function')$('folderListPicker').showPicker();else $('folderListPicker').click()}catch(e){try{$('folderListPicker').click()}catch{}status('Could not open the local folder picker: '+e.message,'warn')}return}try{rootHandle=await window.showDirectoryPicker({mode:workspaceMode==='write'?'readwrite':'read',id:'triforce-workspace'});fallbackFiles.clear();fallbackFileList=null;fallbackRoot='';$('folderName').textContent=(rootHandle.name||'Local folder')+' · '+(workspaceMode==='write'?'read/write':'read-only')+' handle';$('connectBtn').disabled=false;status('Folder selected. No files have been read yet.','ok');$('analysis').textContent='Local directory handle ready. File contents are read only on explicit AI tool requests.';}catch(e){if(e.name!=='AbortError')status('Folder selection failed: '+e.message,'warn')}}
function acceptFolderFileList(files){fallbackFiles.clear();rootHandle=null;fallbackFileList=files||null;const count=Number(fallbackFileList?.length||0);if(!count)return false;const first=fallbackFileList.item?fallbackFileList.item(0):fallbackFileList[0];const firstRel=String(first?.webkitRelativePath||first?.name||'');fallbackRoot=firstRel.includes('/')?firstRel.split('/')[0]:'Selected folder';$('folderName').textContent=fallbackRoot+' · '+count+' files listed';document.querySelector('input[name=mode][value=read_only]').checked=true;$('connectBtn').disabled=false;status('Folder selected. Only the local FileList is stored; no file contents were read.','ok');$('analysis').textContent=count+' local file references available. Path indexing is deferred until the AI requests a file tool.';return true;}
function installFallbackFiles(files,source='drag and drop'){fallbackFileList=null;fallbackFiles.clear();rootHandle=null;const list=[...(files||[])];if(!list.length)return false;const firstRel=String(list[0].webkitRelativePath||list[0]._relativePath||'');fallbackRoot=firstRel.includes('/')?firstRel.split('/')[0]:'Dropped folder';for(const f of list){let p=String(f.webkitRelativePath||f._relativePath||f.name||'');if(fallbackRoot!=='Dropped folder'&&p.startsWith(fallbackRoot+'/'))p=p.slice(fallbackRoot.length+1);p=cleanPath(p||f.name);if(p)fallbackFiles.set(p,f)}if(!fallbackFiles.size)return false;$('folderName').textContent=fallbackRoot+' · '+fallbackFiles.size+' local files';document.querySelector('input[name=mode][value=read_only]').checked=true;$('connectBtn').disabled=false;status('Local folder shared read-only by '+source+'. Nothing was uploaded.','ok');$('analysis').textContent=fallbackFiles.size+' local file references are available only while this browser tab stays open.';return true;}
$('folderListPicker').addEventListener('change',e=>{if(!acceptFolderFileList(e.currentTarget.files))status('The browser returned an empty folder selection.','warn')});
$('writeMode').addEventListener('change',()=>{if($('writeMode').checked&&!modern){document.querySelector('input[name=mode][value=read_only]').checked=true;status('Write is unavailable in this browser because it cannot provide a writable directory handle.','warn');return}if($('writeMode').checked&&rootHandle){status('Write selected. Re-select the folder to request read/write permission immediately, or Connect to request permission now.','warn')}});
async function filesFromEntry(entry,prefix='',out=[]){if(entry.isFile){await new Promise(resolve=>entry.file(f=>{try{Object.defineProperty(f,'_relativePath',{value:(prefix?prefix+'/':'')+f.name})}catch{}out.push(f);resolve()},()=>resolve()));return out}if(entry.isDirectory){const reader=entry.createReader();for(;;){const batch=await new Promise(resolve=>reader.readEntries(resolve,()=>resolve([])));if(!batch.length)break;for(const child of batch)await filesFromEntry(child,prefix?prefix+'/'+entry.name:entry.name,out)}}return out}
function acceptDirectoryHandle(handle,label='Dropped folder'){rootHandle=handle;fallbackFiles.clear();fallbackFileList=null;fallbackRoot='';$('folderName').textContent=(handle.name||label)+' · shared locally';$('connectBtn').disabled=false;status('Local folder shared from drag-and-drop. Nothing was uploaded.','ok');$('analysis').textContent='A real local directory handle was provided by the browser.';return true}
const dz=$('dropZone');for(const ev of ['dragenter','dragover'])dz.addEventListener(ev,e=>{e.preventDefault();if(e.dataTransfer)e.dataTransfer.dropEffect='copy';dz.style.borderColor='#58a6ff'});dz.addEventListener('dragleave',()=>dz.style.borderColor='');dz.addEventListener('drop',async e=>{e.preventDefault();e.stopPropagation();dz.style.borderColor='';const dt=e.dataTransfer;const items=[...(dt?.items||[])];const fileCount=(dt?.files||[]).length;$('analysis').textContent='Drop received: items='+items.length+' files='+fileCount;status('Reading dropped local folder…');
const handlePromises=items.map(i=>typeof i.getAsFileSystemHandle==='function'?i.getAsFileSystemHandle().catch(()=>null):Promise.resolve(null));
const legacyRoots=items.map(i=>{try{const fn=i.getAsEntry||i.webkitGetAsEntry;return typeof fn==='function'?fn.call(i):null}catch{return null}}).filter(Boolean);
const handles=(await Promise.all(handlePromises)).filter(Boolean);const directoryHandle=handles.find(h=>h.kind==='directory');if(directoryHandle){acceptDirectoryHandle(directoryHandle);return}
if(legacyRoots.length){const files=[];for(const entry of legacyRoots)await filesFromEntry(entry,'',files);if(installFallbackFiles(files,'drag and drop'))return}
if(installFallbackFiles([...(dt?.files||[])],'drag and drop'))return;
const kinds=items.map(i=>i.kind+':'+(i.type||'-')).join(', ')||'none';$('analysis').textContent='Drop diagnostics: items='+items.length+' files='+fileCount+' kinds='+kinds+' modernHandles='+handles.length+' legacyEntries='+legacyRoots.length;status('The browser received the drop but did not expose the folder contents or a directory handle. This is a browser/desktop integration limitation, not a TriForce upload or server failure.','warn')});
async function connect(){if(!rootHandle&&!fallbackFiles.size&&!Number(fallbackFileList?.length||0))return;workspaceMode=document.querySelector('input[name=mode]:checked').value;if(workspaceMode==='write'){if(!modern||!rootHandle){status('Write mode is not supported by this browser picker.','warn');return}let perm=await rootHandle.queryPermission({mode:'readwrite'});if(perm!=='granted')perm=await rootHandle.requestPermission({mode:'readwrite'});if(perm!=='granted'){status('Write permission was not granted.','warn');return}}capabilities=[...READ_TOOLS,...(workspaceMode==='write'&&rootHandle?WRITE_TOOLS:[])];status('Creating one-time workspace ID…');$('connectBtn').disabled=true;let ticket;try{const r=await fetch('/v1/mcp/workspace/pair-ticket',{method:'POST',headers:{'Accept':'application/json'}});if(!r.ok)throw new Error('ticket request failed: '+r.status);ticket=await r.json();pairCode=String(ticket.pair_code||'');if(!pairCode)throw new Error('server returned no pairing ID')}catch(e){status('Could not create pairing ID: '+e.message,'warn');$('connectBtn').disabled=false;return}status('Connecting browser workspace to TriForce…');const proto=location.protocol==='https:'?'wss:':'ws:';const url=proto+'//'+location.host+'/v1/mcp/node/connect?mode=workspace&pair_code='+encodeURIComponent(pairCode)+'&machine_id=browser&client_version=2.85-browser';ws=new WebSocket(url);ws.onmessage=async e=>{try{await handleServer(JSON.parse(e.data))}catch(err){status('Workspace error: '+err.message,'warn')}};ws.onerror=()=>status('WebSocket connection failed.','warn');ws.onclose=()=>{if(ws){status('Workspace disconnected. Choose Connect again for a new pairing ID.','warn');$('disconnectBtn').disabled=true}ws=null;};}
function disconnect(){if(ws){try{ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/revoke',params:{}}));ws.close()}catch{}ws=null}$('disconnectBtn').disabled=true;if(rootHandle||fallbackFiles.size||Number(fallbackFileList?.length||0))$('connectBtn').disabled=false;$('pairPanel').classList.add('hidden');}
