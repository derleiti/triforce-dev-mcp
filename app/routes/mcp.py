...
<title>AILinux Helper · TriForce MCP</title><link rel="icon" type="image/png" href="/v1/mcp/helper/icon.png?v=29020"><link rel="apple-touch-icon" href="/v1/mcp/helper/icon.png?v=29020"><link rel="manifest" href="/v1/mcp/manifest.webmanifest?v=29020"><meta name="theme-color" content="#0d1117"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><meta name="apple-mobile-web-app-title" content="AILinux Helper">
<style>
...
const EXECUTOR_VERSION='2.90.20-browser';
...
async def download_android_workspace_helper() -> Response:
    ...
    if not os.path.isfile(apk_path):
        return JSONResponse(
            {"error": "Android helper APK is not built yet", "build": "android_workspace", "version": "2.90.20"},
            status_code=404,
            headers={"Cache-Control": "no-store"},
        )
    return FileResponse(
        ...
        filename="AILinux-Helper-2.90.20-android.apk",
...
async def download_ailinux_helper(platform: str):
    ...
    artifacts = {
        "android": ("AILinux-Helper-latest.apk", "AILinux-Helper-2.90.20-android.apk", "application/vnd.android.package-archive"),
        "linux-appimage": ("AILinux-Helper-latest.AppImage", "AILinux-Helper-2.90.20-linux-x86_64.AppImage", "application/vnd.appimage"),
        "linux-deb": ("AILinux-Helper-latest.deb", "AILinux-Helper-2.90.20-linux-amd64.deb", "application/vnd.debian.binary-package"),
        "windows": ("AILinux-Helper-latest.exe", "AILinux-Helper-2.90.20-win-x64.exe", "application/vnd.microsoft.portable-executable"),
        "macos": ("AILinux-Helper-latest.dmg", "AILinux-Helper-2.90.20-mac-arm64.dmg", "application/x-apple-diskimage"),
    }
    ...
    if not artifact.is_file():
        return JSONResponse(status_code=404, content={"error": f"{platform} AILinux Helper build is pending", "version": "2.90.20", "repository": "ailinux-helper"})
...
async def download_desktop_workspace_helper(platform: str) -> Response:
    ...
    if not os.path.isfile(path):
        return JSONResponse({"error": f"{platform} helper build is pending", "version": "2.90.20", "workflow": "ailinux-helper/build.yml"}, status_code=404, headers={"Cache-Control": "no-store"})
...
async def workspace_pwa_manifest() -> JSONResponse:
    return JSONResponse({
        ...
        "icons": [{"src": "/v1/mcp/helper/icon.png?v=29020", "sizes": "512x512", "type": "image/png", "purpose": "any maskable"}],
...
async def workspace_pwa_service_worker() -> Response:
    script = """'use strict';const CACHE='ailinux-helper-v29020';self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.add('/v1/mcp?app=2.90.20')).catch(()=>{}));self.skipWaiting()});self.addEventListener('activate',e=>{e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&(k.startsWith('ailinux-workspace-')||k.startsWith('ailinux-helper-'))).map(k=>caches.delete(k)))),self.clients.claim()]));});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request).then(r=>r||caches.match('/v1/mcp?app=2.90.20'))))});"""
