...
function scheduleReconnect(){if(manualDisconnect||!pairCode||reconnectTimer||document.hidden||!navigator.onLine)return;const delay=Math.min(10000,500*Math.pow(2,Math.min(reconnectAttempt++,4)));reconnectTimer=setTimeout(async()=>{reconnectTimer=null;await autoResume()},delay)}
