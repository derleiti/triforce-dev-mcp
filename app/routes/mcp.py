...
async function handleServer(msg){lastPongAt=Date.now();if(msg.error&&/(pairing code|workspace credential|resume token)/i.test(String(msg.error))){pairCode='';sessionStorage.removeItem('tf_pair_code');workspaceAttached=false;$('pairPanel').classList.add('hidden');status('Workspace pairing expired. Creating a fresh pairing ID…','warn');try{if(ws)ws.close(4001,'pairing expired')}catch{}setTimeout(()=>connect(),250);return}if(msg.method==='connected'){ws.send(JSON.stringify({jsonrpc:'2.0',method:'client/info',params:{client:'triforce-browser-workspace',platform:navigator.platform||'browser',hostname:'browser',server_version:'2.85-browser',mode:'workspace',workspace:rootHandle?.name||rootEntry?.name||'local',access_mode:workspaceMode,remote_profile:workspaceMode}}));ws.send(JSON.stringify({jsonrpc:'2.0',method:'tools/list',params:{tools:['client_workspace_tool']}}));ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/share',params:{task:$('task').value.trim(),access_mode:workspaceMode,mode:workspaceMode,capabilities}}));return}if(msg.method==='workspace/shared'){if(msg.params?.ok){if(msg.params?.resume_token){resumeToken=String(msg.params.resume_token);await saveWorkspaceState('resumeToken',resumeToken);if(pairCode)await saveWorkspaceState('joinCode',pairCode)}reconnectAttempt=0;lastPongAt=Date.now();workspaceAttached=!msg.params?.waiting_for_session;if(pairCode){$('pair').value=pairCode;$('pairPanel').classList.remove('hidden')}status(workspaceAttached?(msg.params?.reconnected?'Workspace reconnected and AI-reachable.':'Workspace connected and AI-reachable.'):'Browser connected. Waiting for the AI chat to pair this ID.','ok');$('connectBtn').disabled=true;$('disconnectBtn').disabled=false}else status('TriForce rejected workspace: '+(msg.params?.error||'unknown error'),'warn');return}if(msg.method==='workspace/paired'){if(msg.params?.resume_token){resumeToken=String(msg.params.resume_token);await saveWorkspaceState('resumeToken',resumeToken);if(pairCode)await saveWorkspaceState('joinCode',pairCode)}workspaceAttached=true;status('Workspace lease paired; this Join ID remains valid until disconnect or workspace replacement.','ok');return}if(msg.method==='workspace/detached'){workspaceAttached=false;status('AI transport detached; workspace lease is retained. The same pairing ID remains reconnectable.','warn');return}if(msg.method==='tools/call'){let r;const outer=msg.params?.arguments||{},tool=String(outer.tool||''),requestId=String(msg.id||'');try{if(msg.params?.name!=='client_workspace_tool')throw new Error('unexpected tool');ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/tool_stage',params:{request_id:requestId,tool,stage:'started'}}));r=await execute(tool,outer.arguments||{});ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/tool_stage',params:{request_id:requestId,tool,stage:'finished'}}))}catch(e){r=result({ok:false,error:String(e.message||e)},true)}ws.send(JSON.stringify({jsonrpc:'2.0',id:msg.id,result:r}));return}if(msg.method==='ping'){ws.send(JSON.stringify({jsonrpc:'2.0',method:'pong'}));return}if(msg.method==='pong'){lastPongAt=Date.now();return}}
...
function startHeartbeat(){stopHeartbeat();lastPongAt=Date.now();const ping=()=>{if(ws&&ws.readyState===WebSocket.OPEN){try{ws.send(JSON.stringify({jsonrpc:'2.0',method:'ping',params:{ts:Date.now()}}))}catch{}}};ping();heartbeatTimer=setInterval(ping,10000)}
