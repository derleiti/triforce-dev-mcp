...
<div class="row"><button id="chooseBtn">Share local folder</button><span id="folderName" class="muted">No folder shared</span></div>
...
if(!modern){$('writeMode').disabled=true;$('browserNote').textContent='This browser has no direct directory-picker handle API. The button will explain this limitation; desktop folder drag-and-drop can still provide local read-only access when the browser exposes directory entries. Nothing is uploaded.'}else{$('browserNote').textContent='Direct local folder sharing is supported. The folder stays on this device; TriForce requests individual files only when the AI uses a local tool. Nothing is uploaded as a project.'}
$('browserCaps').textContent='Browser filesystem support: picker='+(modern?'yes':'no')+' · drop-handle='+(dropHandleCap?'yes':'no')+' · drop-entry='+(dropEntryCap?'yes':'no');
...
async function chooseFolder(){disconnect();$('pairPanel').classList.add('hidden');if(!modern){status('This browser cannot open a local folder handle from a button. On desktop, drag the project folder into the box below; for full Read/Write use a browser with showDirectoryPicker support.','warn');return}try{rootHandle=await window.showDirectoryPicker({mode:'read',id:'triforce-workspace'});fallbackFiles.clear();fallbackRoot='';$('folderName').textContent=(rootHandle.name||'Local folder')+' · shared locally';$('connectBtn').disabled=false;status('Local folder shared with this browser tab. Choose access and connect.','ok');$('analysis').textContent='The folder remains on this device. No project upload was performed.';}catch(e){if(e.name!=='AbortError')status('Folder sharing failed: '+e.message,'warn')}}
function installFallbackFiles(files){fallbackFiles.clear();rootHandle=null;const list=[...(files||[])];if(!list.length)return false;const firstRel=String(list[0]._relativePath||'');fallbackRoot=firstRel.includes('/')?firstRel.split('/')[0]:'Dropped folder';for(const f of list){let p=String(f._relativePath||f.name||'');if(fallbackRoot!=='Dropped folder'&&p.startsWith(fallbackRoot+'/'))p=p.slice(fallbackRoot.length+1);p=cleanPath(p||f.name);if(p)fallbackFiles.set(p,f)}if(!fallbackFiles.size)return false;$('folderName').textContent=fallbackRoot+' · '+fallbackFiles.size+' local files';document.querySelector('input[name=mode][value=read_only]').checked=true;$('connectBtn').disabled=false;status('Local folder shared read-only by drag and drop. Nothing was uploaded.','ok');$('analysis').textContent=fallbackFiles.size+' local file references are available only while this browser tab stays open.';return true;}
...
if(legacyRoots.length){const files=[];for(const entry of legacyRoots)await filesFromEntry(entry,'',files);if(installFallbackFiles(files))return}
if(installFallbackFiles([...(dt?.files||[])]))return;
