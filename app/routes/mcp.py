...
def _service_mcp_auth_challenge(request: Request):
    if str(request.url.path or "").rstrip("/") != "/v1/mcp/service":
        return None
    if request.headers.get("Authorization", "").strip() or request.headers.get("X-MCP-Token", "").strip():
        return None
    return JSONResponse(
        status_code=401,
        content={"detail": "Authentication required"},
        headers={"WWW-Authenticate": 'Bearer realm="AILinux MCP"'},
    )
...
<title>AILinux Helper · TriForce MCP</title><link rel="icon" type="image/png" href="/v1/mcp/helper/icon.png?v=2902"><link rel="apple-touch-icon" href="/v1/mcp/helper/icon.png?v=2902"><link rel="manifest" href="/v1/mcp/manifest.webmanifest?v=2902"><meta name="theme-color" content="#0d1117"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><meta name="apple-mobile-web-app-title" content="AILinux Helper">
<style>
...
const EXECUTOR_VERSION='2.90.2-browser';
...
const nativeHelper=window.ailinuxHelper||null;let nativeHelperCaps={};
async function nativeHelperTools(){if(!nativeHelper||typeof nativeHelper.getCapabilities!=='function')return [];try{nativeHelperCaps=await nativeHelper.getCapabilities()||{};return HELPER_DEVICE_TOOLS.filter(name=>nativeHelperCaps[name]===true)}catch(e){nativeHelperCaps={};return []}}
async function refreshNativeHelperStatus(){const tools=await nativeHelperTools();if(nativeHelper){$('helperNote').textContent='Native device bridge active · shared MCP capabilities: '+(tools.length?tools.join(', '):'none (enable them from the Helper tray menu)')+'. Mouse/keyboard control stays disabled until a native accessibility backend is available.'}return tools}
...
function setupMobileUi(){const linux=/Linux/i.test(navigator.platform||'')&&!android,mac=/Mac/i.test(navigator.platform||'')&&!ios,win=/Win/i.test(navigator.platform||'');if(android){$('openAppBtn').classList.remove('hidden');$('androidDownload').classList.remove('hidden');$('linuxAppImage').classList.add('hidden');$('linuxDeb').classList.add('hidden');$('windowsDownload').classList.add('hidden');$('macDownload').classList.add('hidden');$('helperText').textContent='Android uses the native foreground executor so the workspace can remain active after the UI is closed. Connection state and reconnect/disconnect actions stay in the system notification.';}else if(ios){$('linuxAppImage').classList.add('hidden');$('linuxDeb').classList.add('hidden');$('windowsDownload').classList.add('hidden');$('macDownload').classList.add('hidden');$('helperText').textContent='On iPhone/iPad install this page as the AILinux Helper web app. The durable lease survives transport suspension and reconnects when iOS allows the app to resume.';$('helperNote').textContent='iOS may suspend background networking and cannot guarantee an arbitrary WebSocket after force-quit.';}else if(linux){$('windowsDownload').classList.add('hidden');$('macDownload').classList.add('hidden');}else if(win){$('linuxAppImage').classList.add('hidden');$('linuxDeb').classList.add('hidden');$('macDownload').classList.add('hidden');}else if(mac){$('linuxAppImage').classList.add('hidden');$('linuxDeb').classList.add('hidden');$('windowsDownload').classList.add('hidden');}if('serviceWorker' in navigator)navigator.serviceWorker.register('/v1/mcp/sw.js?v=2902',{scope:'/v1/mcp',updateViaCache:'none'}).catch(()=>{});}
setupMobileUi();
...
function renderTerminalUi(){const panel=$('terminalPanel');if(!panel)return;panel.classList.remove('hidden');if(!native){$('terminalText').textContent='This is a browser tab, so no terminal is possible here. Use the AILinux Workspace desktop app or the Android helper to release a shell.';$('terminalHint').textContent='Workspace file sharing works without a terminal.';return}const s=nativeShell,released=!!(s&&s.released);$('releaseTermBtn').classList.toggle('hidden',released);$('revokeTermBtn').classList.toggle('hidden',!released);$('terminalText').textContent=released?('Terminal released: '+(s.label||s.backend)+(s.sandboxed?' (sandboxed)':' (not sandboxed)')):'The AI has no terminal on this machine.';$('terminalHint').textContent=s?((s.workspace?('Command folder: '+s.workspace+' \u00b7 '):'')+(s.detail||'')):'';}
function setupTerminalUi(){if(!$('terminalPanel'))return;if(!native){renderTerminalUi();return}$('releaseTermBtn').onclick=async()=>{nativeShell=await native.releaseShell();renderTerminalUi();if(nativeShell&&nativeShell.released)status('Terminal released to the AI. Reconnect the workspace so the capability is announced.','ok')};$('revokeTermBtn').onclick=async()=>{nativeShell=await native.revokeShell();renderTerminalUi();status('Terminal revoked. Reconnect the workspace to drop the capability.','warn')};if(native.onShellChanged)native.onShellChanged(s=>{nativeShell=s;renderTerminalUi()});refreshNativeShell();}
setupTerminalUi();
...
async function execute(tool,args){if(!capabilities.includes(tool))throw new Error('tool not available in this workspace: '+tool);if(tool==='workspace_info')return result(await workspaceInfo());if(tool==='computer_observe'||tool==='computer_screenshot'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.screenshot())}if(tool==='clipboard_read'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardRead())}if(tool==='clipboard_write'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardWrite(String(args.text||'')))}if(tool==='file_read'||tool==='code_read')return result({path:cleanPath(args.path),...lineSlice(await readText(args.path),args)});if(tool==='file_tree'||tool==='code_tree')return result(await tree(args));if(tool==='code_search')return result(await search(args,false));if(tool==='code_grep')return result(await search(args,true));if(tool==='file_edit')return result(await editFile(args));if(tool==='directory_create')return result(await createDir(args));if(tool==='workspace_clear')return result(await clearWorkspace(args));if(tool==='file_ops')return result(await fileOps(args));if(tool==='code_edit')return result(await codeEdit(args));if(tool==='shell')return result(await runNativeShell(args));throw new Error('unsupported browser tool');}
...
async def create_browser_workspace_resume_ticket(request: Request) -> JSONResponse:
    """Exchange a durable resume credential for a short-lived WebSocket ticket."""
    from app.services.mcp_workspace_sessions import (
        RESUME_TICKET_TTL_SECONDS, create_workspace_resume_ticket,
    )
    try:
        body = await request.json()
    except Exception:
        body = {}
    token = str(body.get("resume_token") or "").strip() if isinstance(body, dict) else ""
    if not token:
        return JSONResponse({"error": "resume_token required"}, status_code=400, headers={"Cache-Control": "no-store"})
    try:
        ticket = create_workspace_resume_ticket(token)
    except ValueError:
...
@public_router.post("/mcp/workspace/handoff-ticket", tags=["MCP"], summary="Create one-shot browser to app workspace handoff")
async def create_browser_workspace_handoff_ticket(request: Request) -> JSONResponse:
    from app.services.mcp_workspace_sessions import HANDOFF_TICKET_TTL_SECONDS, create_workspace_handoff_ticket
    try:
        body = await request.json()
    except Exception:
        body = {}
    token = str(body.get("resume_token") or "").strip() if isinstance(body, dict) else ""
    target = str(body.get("target") or "app").strip() if isinstance(body, dict) else "app"
    if not token:
        return JSONResponse({"error": "resume_token required"}, status_code=400, headers={"Cache-Control": "no-store"})
    try:
        ticket = create_workspace_handoff_ticket(token, target=target)
    except ValueError:
        return JSONResponse({"error": "invalid or expired workspace session"}, status_code=403, headers={"Cache-Control": "no-store"})
    base = str(request.base_url).rstrip("/")
    return JSONResponse({
        "handoff_code": ticket,
        "handoff_url": f"{base}/v1/mcp/workspace/handoff#code={ticket}",
        "expires_seconds": HANDOFF_TICKET_TTL_SECONDS,
    }, headers={"Cache-Control": "no-store"})
...
async def download_android_workspace_helper() -> Response:
    ...
    if not os.path.isfile(apk_path):
        return JSONResponse(
            {"error": "Android helper APK is not built yet", "build": "android_workspace", "version": "2.90.2"},
            status_code=404,
            headers={"Cache-Control": "no-store"},
        )
    return FileResponse(
        ...
        filename="AILinux-Helper-2.90.2-android.apk",
...
async def download_ailinux_helper(platform: str):
    ...
    artifacts = {
        "android": ("AILinux-Helper-latest.apk", "AILinux-Helper-2.90.2-android.apk", "application/vnd.android.package-archive"),
        "linux-appimage": ("AILinux-Helper-latest.AppImage", "AILinux-Helper-2.90.2-linux-x86_64.AppImage", "application/vnd.appimage"),
        "linux-deb": ("AILinux-Helper-latest.deb", "AILinux-Helper-2.90.2-linux-amd64.deb", "application/vnd.debian.binary-package"),
        "windows": ("AILinux-Helper-latest.exe", "AILinux-Helper-2.90.2-windows-x64.exe", "application/vnd.microsoft.portable-executable"),
        "macos": ("AILinux-Helper-latest.dmg", "AILinux-Helper-2.90.2-macos.dmg", "application/x-apple-diskimage"),
    }
    ...
    if not artifact.is_file():
        return JSONResponse(status_code=404, content={"error": f"{platform} AILinux Helper build is pending", "version": "2.90.2", "repository": "ailinux-helper"})
...
async def download_desktop_workspace_helper(platform: str) -> Response:
    ...
    if not os.path.isfile(path):
        return JSONResponse({"error": f"{platform} helper build is pending", "version": "2.90.2", "workflow": "ailinux-helper/build.yml"}, status_code=404, headers={"Cache-Control": "no-store"})
...
async def workspace_pwa_manifest() -> JSONResponse:
    return JSONResponse({
        ...
        "icons": [{"src": "/v1/mcp/helper/icon.png?v=2902", "sizes": "512x512", "type": "image/png", "purpose": "any maskable"}],
...
async def workspace_pwa_service_worker() -> Response:
    script = """'use strict';const CACHE='ailinux-helper-v2902';self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.add('/v1/mcp?app=2.90.2')).catch(()=>{}));self.skipWaiting()});self.addEventListener('activate',e=>{e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&(k.startsWith('ailinux-workspace-')||k.startsWith('ailinux-helper-'))).map(k=>caches.delete(k)))),self.clients.claim()]));});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request).then(r=>r||caches.match('/v1/mcp?app=2.90.2'))))});"""
...
def _store_session_request_state(session: Dict[str, TypingAny], request: Request) -> None:
    ...
    session["workspace_subject"] = getattr(state, "mcp_workspace_subject", None)
...
def _clear_mcp_session(session_id: str, *, clear_workspace: bool = False) -> None:
    ...
    try:
        if clear_workspace:
            ...
            from app.services.mcp_workspace_sessions import mark_transport_detached
            binding = mark_transport_detached(session_id)
            if binding:
                connection = binding.get("connection")
                if connection is not None and not bool(getattr(connection, "closed", True)):
                    try:
                        loop = asyncio.get_running_loop()
                        loop.create_task(connection.websocket.send_json({
                            "jsonrpc": "2.0", "method": "workspace/detached",
                            "params": {"lease_id": binding.get("lease_id", ""), "rebindable": True},
                        }))
                    except RuntimeError:
                        pass
    except Exception:
...
def _session_timestamp_utc(value: TypingAny) -> dt_datetime:
    """Normalize current and pre-UTC session timestamps for safe age checks."""
...
def _cleanup_old_sessions():
    ...
    expired = [
        ...
        if (now - _session_timestamp_utc(data.get("last_seen", data["created"]))).total_seconds() > 3600
    ]
    for sid in expired:
...
@router.get("/mcp/", tags=["MCP"], summary="MCP health check or SSE endpoint")
@public_router.get("/mcp/service", tags=["MCP"], summary="Authenticated MCP service bridge endpoint")
@public_router.get("/mcp/service/", tags=["MCP"], summary="Authenticated MCP service bridge endpoint")
async def mcp_health_or_sse(request: Request):
    """MCP transport for machines, setup page for ordinary browser GET requests."""
    challenge = _service_mcp_auth_challenge(request)
...
async def mcp_sse_connect(request: Request):
    ...
    session["last_seen"] = dt_datetime.now(timezone.utc)
...
@router.delete("/mcp/", tags=["MCP"], summary="Terminate MCP session")
@public_router.delete("/mcp/service", tags=["MCP"], summary="Terminate authenticated MCP service session")
@public_router.delete("/mcp/service/", tags=["MCP"], summary="Terminate authenticated MCP service session")
