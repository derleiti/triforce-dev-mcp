...
if(!modern){$('writeMode').disabled=true;$('chooseBtn').disabled=true;$('browserNote').textContent='This browser cannot grant a web page a direct local folder handle. Nothing will be uploaded. On desktop you can drag a project folder into the local share box below for read-only access; for full Read/Write use a browser that supports direct directory handles.'}else{$('browserNote').textContent='Direct local folder sharing is supported. The folder stays on this device; TriForce requests individual files only when the AI uses a local tool. Nothing is uploaded as a project.'}
