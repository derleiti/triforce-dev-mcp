...
<title>AILinux Helper · TriForce MCP</title><link rel="icon" type="image/png" href="/v1/mcp/helper/icon.png?v=29013"><link rel="apple-touch-icon" href="/v1/mcp/helper/icon.png?v=29013"><link rel="manifest" href="/v1/mcp/manifest.webmanifest?v=29013"><meta name="theme-color" content="#0d1117"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><meta name="apple-mobile-web-app-title" content="AILinux Helper">
<style>
...
const HELPER_DEVICE_TOOLS=['computer_observe','computer_screenshot','clipboard_read','clipboard_write','compute_execute'];
const IGNORE=new Set(['.git','.venv','node_modules','__pycache__','.pytest_cache','.mypy_cache']);
const MAX_TEXT=2*1024*1024;
const EXECUTOR_VERSION='2.90.13-browser';
...
async function revokeAllShare(){if(nativeHelper&&typeof nativeHelper.setShareProfile==='function'){nativeShareProfile=await nativeHelper.setShareProfile({clipboardRead:false,clipboardWrite:false,screenObserve:false,resourceAdvertise:false,computeAdvertise:false,mcpAdvertise:false})||{};}capabilities=[];renderShareSummary();await disconnect(true);syncNativeShareUi();status('All workspace and native capability grants revoked.','warn')}
...
async function execute(tool,args){if(!capabilities.includes(tool))throw new Error('tool not available in this workspace: '+tool);if(tool==='workspace_info')return result(await workspaceInfo());if(tool==='computer_observe'||tool==='computer_screenshot'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.screenshot())}if(tool==='compute_execute'){if(!nativeHelper||typeof nativeHelper.runCompute!=='function')throw new Error('native compute bridge unavailable');return result(await nativeHelper.runCompute(args))}if(tool==='shell'){if(native)return result(await runNativeShell(args));throw new Error('native shell unavailable')}if(tool==='clipboard_read'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardRead())}if(tool==='clipboard_write'){if(!nativeHelper)throw new Error('native AILinux Helper bridge unavailable');return result(await nativeHelper.clipboardWrite(String(args.text||'')))}if(tool==='file_read'||tool==='code_read')return result({path:cleanPath(args.path),...lineSlice(await readText(args.path),args)});if(tool==='file_tree'||tool==='code_tree')return result(await tree(args));if(tool==='code_search')return result(await search(args,false));if(tool==='code_grep')return result(await search(args,true));if(tool==='file_edit')return result(await editFile(args));if(tool==='directory_create')return result(await createDir(args));if(tool==='workspace_clear')return result(await clearWorkspace(args));if(tool==='file_ops')return result(await fileOps(args));if(tool==='code_edit')return result(await codeEdit(args));throw new Error('unsupported browser tool');}
...
async def workspace_pwa_manifest() -> JSONResponse:
    return JSONResponse({
        ...
        "icons": [{"src": "/v1/mcp/helper/icon.png?v=29013", "sizes": "512x512", "type": "image/png", "purpose": "any maskable"}],
...
async def workspace_pwa_service_worker() -> Response:
    script = """'use strict';const CACHE='ailinux-helper-v29013';self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.add('/v1/mcp?app=2.90.13')).catch(()=>{}));self.skipWaiting()});self.addEventListener('activate',e=>{e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&(k.startsWith('ailinux-workspace-')||k.startsWith('ailinux-helper-'))).map(k=>caches.delete(k)))),self.clients.claim()]));});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request).then(r=>r||caches.match('/v1/mcp?app=2.90.13'))))});"""
