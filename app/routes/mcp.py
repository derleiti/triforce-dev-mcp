...
$('chooseBtn').onclick=chooseFolder;$('connectBtn').onclick=connect;$('disconnectBtn').onclick=disconnect;$('copyBtn').onclick=()=>navigator.clipboard.writeText(pairCode);$('copyMcp').onclick=()=>navigator.clipboard.writeText($('mcpUrl').value);document.addEventListener('visibilitychange',()=>{if(!document.hidden)autoResume()});window.addEventListener('pageshow',()=>autoResume());window.addEventListener('online',()=>autoResume());window.addEventListener('pagehide',()=>{if(ws)try{ws.close()}catch{}});restoreSavedWorkspace();
