...
<div class="row" style="margin-top:14px"><button id="chooseBtn">Choose local folder</button><span id="folderName" class="muted">No folder shared</span></div>
...
let pairCode='',rootHandle=null,rootEntry=null,ws=null,workspaceMode='read_only',capabilities=[];
...
if(!directPicker){$('writeMode').disabled=true;$('browserNote').textContent=(android?'Android: ':'')+(firefox?'Firefox: ':'')+'this browser does not expose showDirectoryPicker(), so full lazy Read/Write workspace selection is unavailable. Use a Chromium browser with File System Access support. Desktop drag-and-drop may still provide lazy read-only access.';}else{$('browserNote').textContent='Direct directory handles are available. Choose Read only or Write, then choose the folder. No file list is built during selection.';}
$('browserCaps').textContent='Browser filesystem support: directory-picker='+(directPicker?'yes':'no')+' · drop-handle='+(dropHandleCap?'yes':'no')+' · drop-entry='+(dropEntryCap?'yes':'no')+' · android='+(android?'yes':'no')+' · firefox='+(firefox?'yes':'no');
...
async function ensureWritePermission(){if(!rootHandle)throw new Error('write requires a real directory handle');const opts={mode:'readwrite'};if(typeof rootHandle.queryPermission==='function'&&await rootHandle.queryPermission(opts)==='granted')return true;if(typeof rootHandle.requestPermission==='function'&&await rootHandle.requestPermission(opts)==='granted')return true;throw new Error('write permission was not granted by the browser');}
...
async function handleServer(msg){if(msg.method==='connected'){ws.send(JSON.stringify({jsonrpc:'2.0',method:'client/info',params:{client:'triforce-browser-workspace',platform:navigator.platform||'browser',hostname:'browser',server_version:'2.85-browser',mode:'workspace',workspace:rootHandle?.name||rootEntry?.name||'local',remote_profile:workspaceMode}}));ws.send(JSON.stringify({jsonrpc:'2.0',method:'tools/list',params:{tools:['client_workspace_tool']}}));ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/share',params:{task:$('task').value.trim(),mode:workspaceMode,capabilities}}));return}if(msg.method==='workspace/shared'){if(msg.params?.ok){$('pair').value=pairCode;$('pairPanel').classList.remove('hidden');status('Workspace ready. Paste the ID into your AI chat.','ok');$('connectBtn').disabled=true;$('disconnectBtn').disabled=false}else status('TriForce rejected workspace: '+(msg.params?.error||'unknown error'),'warn');return}if(msg.method==='tools/call'){let r;try{if(msg.params?.name!=='client_workspace_tool')throw new Error('unexpected tool');const outer=msg.params.arguments||{};r=await execute(String(outer.tool||''),outer.arguments||{})}catch(e){r=result({ok:false,error:String(e.message||e)},true)}ws.send(JSON.stringify({jsonrpc:'2.0',id:msg.id,result:r}));return}if(msg.method==='ping')ws.send(JSON.stringify({jsonrpc:'2.0',method:'pong'}));}
async function connect(){if(!rootHandle&&!rootEntry)return;workspaceMode=rootEntry?'read_only':document.querySelector('input[name=mode]:checked').value;if(workspaceMode==='write')await ensureWritePermission();capabilities=[...READ_TOOLS,...(workspaceMode==='write'&&rootHandle?WRITE_TOOLS:[])];status('Creating one-time workspace ID…');$('connectBtn').disabled=true;try{const r=await fetch('/v1/mcp/workspace/pair-ticket',{method:'POST',headers:{Accept:'application/json'}});if(!r.ok)throw new Error('ticket request failed: '+r.status);const ticket=await r.json();pairCode=String(ticket.pair_code||'');if(!pairCode)throw new Error('server returned no pairing ID')}catch(e){status('Could not create pairing ID: '+e.message,'warn');$('connectBtn').disabled=false;return}status('Connecting browser workspace to TriForce…');const proto=location.protocol==='https:'?'wss:':'ws:';ws=new WebSocket(proto+'//'+location.host+'/v1/mcp/node/connect?mode=workspace&pair_code='+encodeURIComponent(pairCode)+'&machine_id=browser&client_version=2.85-browser');ws.onmessage=async e=>{try{await handleServer(JSON.parse(e.data))}catch(err){status('Workspace error: '+err.message,'warn')}};ws.onerror=()=>status('WebSocket connection failed.','warn');ws.onclose=()=>{ws=null;$('disconnectBtn').disabled=true;if(rootHandle||rootEntry)$('connectBtn').disabled=false};}
function disconnect(){if(ws){try{ws.send(JSON.stringify({jsonrpc:'2.0',method:'workspace/revoke',params:{}}));ws.close()}catch{}ws=null}$('disconnectBtn').disabled=true;if(rootHandle||rootEntry)$('connectBtn').disabled=false;$('pairPanel').classList.add('hidden');}
