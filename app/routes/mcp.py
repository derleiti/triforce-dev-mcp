...
async function chooseFolder(){disconnect();$('pairPanel').classList.add('hidden');if(!modern){$('folderListPicker').value='';$('folderListPicker').click();return}try{rootHandle=await window.showDirectoryPicker({mode:'read',id:'triforce-workspace'});fallbackFiles.clear();fallbackRoot='';$('folderName').textContent=(rootHandle.name||'Local folder')+' · directory handle';$('connectBtn').disabled=false;status('Folder selected. No files have been read yet.','ok');$('analysis').textContent='Local directory handle ready. File contents are read only on explicit AI tool requests.';}catch(e){if(e.name!=='AbortError')status('Folder selection failed: '+e.message,'warn')}}
function installFallbackFiles(files,source='folder picker'){fallbackFiles.clear();rootHandle=null;const list=[...(files||[])];if(!list.length)return false;const firstRel=String(list[0].webkitRelativePath||list[0]._relativePath||'');fallbackRoot=firstRel.includes('/')?firstRel.split('/')[0]:'Selected folder';for(const f of list){let p=String(f.webkitRelativePath||f._relativePath||f.name||'');if(fallbackRoot!=='Selected folder'&&p.startsWith(fallbackRoot+'/'))p=p.slice(fallbackRoot.length+1);p=cleanPath(p||f.name);if(p)fallbackFiles.set(p,f)}if(!fallbackFiles.size)return false;$('folderName').textContent=fallbackRoot+' · '+fallbackFiles.size+' files indexed';document.querySelector('input[name=mode][value=read_only]').checked=true;$('connectBtn').disabled=false;status('Folder selected. File list indexed locally; no file contents were read.','ok');$('analysis').textContent=fallbackFiles.size+' paths indexed locally via '+source+'. No analysis has run.';return true;}
$('folderListPicker').addEventListener('change',e=>{if(!installFallbackFiles(e.currentTarget.files,'folder picker'))status('The browser returned an empty folder selection.','warn')});
