...
def _helper_design_css() -> str:
    """Render CSS variables from Loom-generated semantic tokens.

    The JSON snapshot is generated by AILinux Loom and includes a deterministic
    fingerprint; TriForce does not own a second palette.
    """
    token_path = os.path.join(os.path.dirname(__file__), "..", "ui", "design_tokens.json")
    try:
        ...
        dark = light = {
            "background": "#071018", "surface": "#0a151e", "glass": "rgba(17,33,44,0.80)",
            "glass-border": "rgba(126,220,255,0.19)", "accent": "#55e7d0",
            "accent-hover": "#79f0dc", "text": "#e9f3ff", "text-muted": "#7996a9",
            "success": "#69e5b1", "warning": "#ffd27a", "error": "#ff7f96",
            "radius": "14px", "spacing": "12px", "typography": "Inter, Noto Sans, sans-serif",
            "shadow": "0 16px 48px rgba(0,0,0,0.24)",
        }
    def variables(values: dict) -> str:
        return ";".join(f"--ailinux-{key}:{value}" for key, value in values.items())
