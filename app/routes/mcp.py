...
<title>AILinux Helper · TriForce MCP</title><link rel="icon" type="image/png" href="/v1/mcp/helper/icon.png?v=29027"><link rel="apple-touch-icon" href="/v1/mcp/helper/icon.png?v=29027"><link rel="manifest" href="/v1/mcp/manifest.webmanifest?v=29027"><meta name="theme-color" content="#0d1117"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"><meta name="apple-mobile-web-app-title" content="AILinux Helper">
<style>
...
const EXECUTOR_VERSION='2.90.27-browser';
...
async def workspace_pwa_manifest() -> JSONResponse:
    return JSONResponse({
        ...
        "icons": [{"src": "/v1/mcp/helper/icon.png?v=29027", "sizes": "512x512", "type": "image/png", "purpose": "any maskable"}],
...
async def workspace_pwa_service_worker() -> Response:
    script = """'use strict';const CACHE='ailinux-helper-v29027';self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.add('/v1/mcp?app=2.90.27')).catch(()=>{}));self.skipWaiting()});self.addEventListener('activate',e=>{e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&(k.startsWith('ailinux-workspace-')||k.startsWith('ailinux-helper-'))).map(k=>caches.delete(k)))),self.clients.claim()]));});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request).then(r=>r||caches.match('/v1/mcp?app=2.90.27'))))});"""
