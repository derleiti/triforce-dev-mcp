...
if(!modern){$('writeMode').disabled=true;$('browserNote').textContent='Read-only folder selection uses the browser FileList API. The selected files stay local in this tab; TriForce does not upload the project. Only a file explicitly requested later by an AI tool is read and returned.'}else{$('browserNote').textContent='Direct local folder sharing is supported. Selecting the folder only creates a local file index; no project analysis runs and nothing is uploaded.'}
$('browserCaps').textContent='Browser filesystem support: direct-handle='+(modern?'yes':'no')+' · local-filelist='+('webkitdirectory' in $('folderListPicker')?'yes':'no')+' · drop-handle='+(dropHandleCap?'yes':'no')+' · drop-entry='+(dropEntryCap?'yes':'no');
...
async function chooseFolder(){disconnect();$('pairPanel').classList.add('hidden');if(!modern){$('folderListPicker').value='';$('folderListPicker').click();return}try{rootHandle=await window.showDirectoryPicker({mode:'read',id:'triforce-workspace'});fallbackFiles.clear();fallbackFileList=null;fallbackRoot='';$('folderName').textContent=(rootHandle.name||'Local folder')+' · directory handle';$('connectBtn').disabled=false;status('Folder selected. No files have been read yet.','ok');$('analysis').textContent='Local directory handle ready. File contents are read only on explicit AI tool requests.';}catch(e){if(e.name!=='AbortError')status('Folder selection failed: '+e.message,'warn')}}
