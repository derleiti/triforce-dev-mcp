...
TASKS: dict[str, SetupTask] = {
    ...
    "profile-server": SetupTask("profile-server", "Als Server einrichten", "Richtet TriForce als dauerhaft laufenden Server ein: Runtime, Konfiguration, systemd-Dienst, Autostart und Docker Engine/Compose v2. Docker-Stacks selbst bleiben gestoppt.", True, True, ("system-check",), ("TriForce Runtime/Konfiguration einrichten", "TRIFORCE_DEPLOYMENT_MODE=server setzen", "Server auf 0.0.0.0:9000 bereitstellen", "triforce.service installieren/reparieren", "triforce.service aktivieren und starten", "Docker Engine und Compose v2 bei Bedarf installieren", "docker.service aktivieren und starten", "keine Docker-Stacks automatisch starten"), "Idempotentes Komplettprofil; vorhandene Secrets bleiben erhalten. Das Server-Profil setzt den Backend-Endpunkt bewusst auf 0.0.0.0:9000.", check_server_profile, "profile-server"),
