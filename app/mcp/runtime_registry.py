...
class RuntimeToolRegistry:
    ...
    def _policy_defaults(self, name: str, inventory: str, source: str) -> Dict[str, Any]:
        ...
        read_only = not (requires_file_write or requires_shell or destructive or name in CLIPBOARD_WRITE_TOOLS or name in {"workspace_pair", "memory_history", "prompt_set", "config_set", "service_control", "container_control", "package_manager", "vault_add", "mail_send", "wp_create_draft", "wp_update_post", "flarum_post_create", "flarum_post_edit", "flarum_discussion_create", "flarum_admin_request", "notify_send", "notify_clear", "notify_read", "idle_assign"})
    ...
    def _classify_tool(self, name: str, defaults: Dict[str, Any]) -> str:
        ...
        if name in WORKSPACE_WRITE_TOOLS or name in CLIPBOARD_WRITE_TOOLS or name in {"workspace_pair", "file_write", "file_ops", "code_patch", "codebase_edit", "codebase_create", "wp_create_draft", "wp_update_post", "mail_send", "flarum_post_create", "flarum_post_edit", "flarum_discussion_create", "notify_send", "notify_clear", "notify_read", "idle_assign"}:
            return "write_scoped"
