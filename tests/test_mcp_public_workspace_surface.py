...
def test_browser_workspace_page_contains_direct_folder_runtime_without_helper_uri():
    ...
    assert 'AILinux Helper 2.90.14' in html
    ...
    assert '/v1/mcp/helper/icon.png?v=29014' in html
    ...
    assert "EXECUTOR_VERSION='2.90.14-browser'" in html
...
def test_mobile_workspace_install_surface_and_pwa_contract():
    ...
    assert '/v1/mcp/manifest.webmanifest?v=29014' in html
    assert "/v1/mcp/sw.js?v=29014" in html
...
async def test_workspace_pwa_routes_have_installable_metadata_and_offline_shell():
    ...
    assert b"ailinux-helper-v29014" in worker.body
    assert b"caches.match('/v1/mcp?app=2.90.14')" in worker.body
    assert b'ailinux-helper-v29014' in worker.body
