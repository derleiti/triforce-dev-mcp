...
def test_browser_workspace_page_contains_direct_folder_runtime_without_helper_uri():
    ...
    assert 'AILinux Helper 2.90.20' in html
    ...
    assert '/v1/mcp/helper/icon.png?v=29020' in html
    ...
    assert "EXECUTOR_VERSION='2.90.20-browser'" in html
...
def test_mobile_workspace_install_surface_and_pwa_contract():
    ...
    assert '/v1/mcp/manifest.webmanifest?v=29020' in html
    assert "/v1/mcp/sw.js?v=29020" in html
...
async def test_workspace_pwa_routes_have_installable_metadata_and_offline_shell():
    ...
    assert b"ailinux-helper-v29020" in worker.body
    assert b"caches.match('/v1/mcp?app=2.90.20')" in worker.body
    assert b'ailinux-helper-v29020' in worker.body
...
def test_linux_helper_download_metadata_tracks_published_29020_build():
    import inspect
    ...
    for source in (public_source, desktop_source):
        assert "AILinux-Helper-2.90.20-linux-x86_64.AppImage" in source
        assert "AILinux-Helper-2.90.20-linux-amd64.deb" in source
