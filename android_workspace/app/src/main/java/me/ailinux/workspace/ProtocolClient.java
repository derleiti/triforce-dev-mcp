...
final class ProtocolClient extends WebSocketListener {
    ...
    private volatile WebSocket ws; private volatile String handoffCode=""; private volatile int reconnectAttempt=0; private final AtomicBoolean stopped=new AtomicBoolean(false);
    ...
    @Override public void onClosed(WebSocket socket,int code,String reason){ws=null;if(!stopped.get())scheduleReconnect("Disconnected ("+code+")");}
    @Override public void onFailure(WebSocket socket,Throwable t,Response response){ws=null;if(!stopped.get())scheduleReconnect("Connection lost");}
    private void scheduleReconnect(String message){listener.onState(message+" · reconnecting");long delay=Math.min(30,1L<<Math.min(5,reconnectAttempt++));timer.schedule(this::connect,delay,TimeUnit.SECONDS);}
}
